...
def compute_projections(
    ...
    # 1. Determine the full annual P&I payment the owner was originally making
    annual_pi_payment_affordable = initial_avail_mortgage_payment_monthly_affordable * 12
    annual_pi_payment_market = initial_avail_mortgage_payment_monthly_market * 12

    # 2. Calculate actual P&I paid to the bank each year (converts lists to numpy arrays)
    actual_pi_paid_yearly_affordable = np.array(initial_principal_per_period_affordable) + np.array(initial_interest_per_period_affordable)
    actual_pi_paid_yearly_market = np.array(initial_principal_per_period_market) + np.array(initial_interest_per_period_market)

    # 3. Any amount of the normal P&I *not* paid to the bank is cash kept in the owner's pocket
    freed_cash_yearly_affordable = np.maximum(0, annual_pi_payment_affordable - actual_pi_paid_yearly_affordable)
    freed_cash_yearly_market = np.maximum(0, annual_pi_payment_market - actual_pi_paid_yearly_market)

    # 4. Accumulate these liquid savings over time
    cumulative_freed_cash_affordable = np.cumsum(freed_cash_yearly_affordable)
    cumulative_freed_cash_market = np.cumsum(freed_cash_yearly_market)
    ...
    for t in range(initial_holding_period_years):
        if using_FHA_loan:
            if initial_downpayment_pct < 10.0:
                realistic_pmi_monthly_array[t] = applied_pmi_monthly  # FHA stays for life of loan
            else:
                realistic_pmi_monthly_array[t] = applied_pmi_monthly if t < 11 else 0.0  # FHA drops off after 11 years
        else:
            # Standard Conventional PMI drop-off happens at 80% LTV of ORIGINAL purchase price
    ...
    resale_net_wealth_built_fixed = gross_appreciation_fixed - buyer_closing_costs_affordable - seller_selling_costs_fixed + principal_repaid_affordable + cumulative_freed_cash_affordable
    resale_net_wealth_built_ami = gross_appreciation_ami - buyer_closing_costs_affordable - seller_selling_costs_ami + principal_repaid_affordable + cumulative_freed_cash_affordable
    resale_net_wealth_built_market = gross_appreciation_market - buyer_closing_costs_market - seller_selling_costs_market + principal_repaid_market + cumulative_freed_cash_market
    ...
    financials_df = pd.DataFrame(
        {
            ...
            "InitialAffordabilityGapAmount": np.round(initial_affordability_gap_amount, 2),
            "InitialRemainingSubsidyRequired": np.round(initial_remaining_subsidy_required, 2),
    ...
    if adjust_for_inflation:
        discount_factors = (1 + general_inflation_rate) ** initial_years
        cols_to_discount = [
            "InitialMarketRateValue", "InitialVALEFixedRateValue", "InitialAMIRateValue",
            "ResaleNetWealthBuiltMarket", "ResaleNetWealthBuiltFixed", "ResaleNetWealthBuiltAMI",
            "ResaleCLTEquityAmountFixed", "ResaleCLTEquityAmountAMI",
            "ResaleCumMortgagePayment_P_And_I_Afforable", "ResaleCumPropertyTaxFixed",
            "ResaleCumInsurance", "ResaleCumHOA", "ResaleCumGroundLease", "ResaleSellingCostsFixed",
            "GrossAppreciationFixed", "PrincipalRepaidAffordable", "BuyerClosingCostsAffordable",
            "SellerClosingCostsFixed"
        ]
        
        for col in cols_to_discount:
            financials_df[col] = np.round(financials_df[col] / discount_factors, 2)
