...
with st.sidebar.expander("**🏡 Property Value - Initial & Ongoing**", expanded=True):
    ...
    initial_affordability_pct_of_ami = st.number_input("Prospective Buyer Income Target (% of AMI)", value=80.0, step=10.0)
...
with st.sidebar.expander("**⚖️ Taxes & Fees**", expanded=True):
    ...
    initial_pmi_financing_fees_monthly = st.number_input("PMI or Additional monthly fees/costs $", value=0.0)
...
with st.sidebar.expander("**🫴🪙Outside Subsidy**", expanded=True):
    initial_downpayment_assistance_amount = st.number_input("Downpayment Assistance $ Amount", value=0, step=10)
    initial_affordability_gap_amount = st.number_input("Affordability Gap $ Amount", value=0, step=10)
    initial_project_gap_amount = st.number_input("Project Gap $ Amount", value=0, step=10)
...
#     try:
#         initial_ami_four_person_dollar_amount = float(df_ami.loc[0, 'AMI'])
#     except Exception:
#         pass
...
initial_downpayment_amount_affordable = float(proj["InitialDownpaymentAmountAffordable"].iloc[0])
...
def render_affordability_subsidy_widget(data: dict):
    ...
    Expected keys in `data` dict:
        - initial_target_income_annual (float)
        - initial_target_income_monthly (float)
        - initial_avail_housing_annual (float)
        - initial_avail_housing_monthly (float)
        - initial_prop_taxes_annual (float)
        - initial_prop_taxes_monthly (float)
        - initial_insurance_annual (float)
        - initial_insurance_monthly (float)
        - initial_hoa_annually (float)
        - initial_hoa_monthly (float)
        - initial_ground_lease_annually (float)
        - initial_ground_lease_monthly (float)
        - initial_pmi_financing_fees_annually (float)
        - initial_pmi_financing_fees_monthly (float)
        - initial_total_other_housing_annual (float)
        - initial_total_other_housing_monthly (float)
        - initial_avail_mortgage_monthly_affordable (float)
        - initial_mortgage_rate_annually_decimal (float)  # e.g., 0.06 for 6.00%
        - initial_max_loan_amount_affordable (float)
        - downpayment (float)
        - outside_gap_loan (float)
        - initial_affordable_purchase_price (float)
        - initial_subsidy_required (float)
    """
    ...
    <div class="sheet-container">
        ...
        <table class="sheet-table">
            ...
            <tr>
                ...
                <td></td>
                <td style="text-align: right;">{f_curr(data['initial_downpayment_affordable'])}</td>
            </tr>
            <!-- Closing Costs -->
            <tr>
                <td>
...
if __name__ == "__main__":
    ...
    sample_financials = {
        ...
        "initial_downpayment_affordable": initial_downpayment_amount_affordable,
...
with col2:
    ...
    #net_proceeds_fig.add_trace(go.Scatter(x=proj['Year'], y=proj['NetCashflowChangeMarket'], mode='lines+markers', name='Equity Gained (Market)', line=dict(color='#3cb44b', width=2),  marker=dict(symbol='diamond', size=8), hovertemplate="Market - Year: %{x}<br>Equity Earned: %{y:$,.0f}<extra></extra>"))
    #net_proceeds_fig.add_trace(go.Scatter(x=proj['Year'], y=proj['NetCashflowChangeFixed'], mode='lines+markers', name='Equity Gained (Fixed)', line=dict(color='#F46A25', width=2),  marker=dict(symbol='star', size=8), hovertemplate="Fixed - Year: %{x}<br>Equity Earned: %{y:$,.0f}<extra></extra>"))
    #net_proceeds_fig.add_trace(go.Scatter(x=proj['Year'], y=proj['NetCashflowChangeAMI'], mode='lines+markers', name='Equity Gained (AMI)', line=dict(color='#4363d8', width=2),  marker=dict(symbol='cross', size=8), hovertemplate="AMI - Year: %{x}<br>Equity Earned: %{y:$,.0f}<extra></extra>"))
...
cols = ['Year', 'InitialMortgageRateMonthly', 'InitialAvailMortgagePaymentMonthlyAffordable', 'InitialVALEFixedRateValue', 'InitialBalanceAffordable', 'InitialInterstYearlyAffordable', 'InitialPrincipalYearlyAffordable', 'InitialDownpaymentAmountAffordable', 'InitialBalanceMarket', 'InitialInterestYearlyMarket', 'InitialPrincipalYearlyMarket', 'InitialMarketRateValue', 'InitialMortgageMarket', 'InitialClosingCostsMarket', 'InitialClosingCostsAffordable', 'InitialMortgageAffordable', 'InitialAMIRateValue', 'InitialPropertyTaxAmountAnnual']
st.dataframe(proj[cols], hide_index=True)
