...
def compute_projections(
    ...
    # ==========================================
    # Notice we add the DPA's "monthly payment equivalent" to the numerator here!

    base_numerator = initial_target_income_monthly_available_for_housing - initial_static_costs_monthly_no_pmi - initial_insurance_amount_monthly + (initial_downpayment_assistance_amount * amortization_factor)
    ...
    # ==========================================
    # 1. Calculate Buyer's cash down (enforcing $500 minimum)
    initial_buyer_cash_contribution_downpayment = max(500.0, math.ceil(P_pass_1 * initial_downpayment_rate_decimal))
    
    # 2. Check total equity percentage
    initial_total_buyer_equity_dollars = initial_buyer_cash_contribution_downpayment + initial_downpayment_assistance_amount
    initial_equity_pct = initial_total_buyer_equity_dollars / P_pass_1 if P_pass_1 > 0 else 0.0

    # 3. Apply PMI logic
    if initial_equity_pct >= 0.20:
        ...
        if initial_downpayment_assistance_covers_buyer_contribution_check:
            # Recalculate cash contribution with the new, slightly lower purchase price
            initial_buyer_cash_contribution_downpayment = max(500.0, (math.ceil(initial_affordable_purchase_price * initial_downpayment_rate_decimal) - initial_downpayment_assistance_amount))
            initial_total_buyer_equity_dollars = initial_buyer_cash_contribution_downpayment + (initial_buyer_cash_contribution_downpayment - initial_downpayment_assistance_amount)
            initial_equity_pct = initial_total_buyer_equity_dollars / P_pass_1 if P_pass_1 > 0 else 0.0
            print(initial_downpayment_assistance_amount)
            initial_total_downpayment_amount_affordable = initial_buyer_cash_contribution_downpayment + initial_downpayment_assistance_amount
        else:
            # Recalculate cash contribution with the new, slightly lower purchase price
            initial_buyer_cash_contribution_downpayment = max(500.0, math.ceil(initial_affordable_purchase_price * initial_downpayment_rate_decimal))
            initial_total_buyer_equity_dollars = initial_buyer_cash_contribution_downpayment + initial_downpayment_assistance_amount
            initial_equity_pct = initial_total_buyer_equity_dollars / P_pass_1 if P_pass_1 > 0 else 0.0
        ...
        initial_affordable_purchase_price = numerator_pass_2 / denominator if (denominator > 0 and numerator_pass_2 > 0) else 0.0
    ...
    initial_max_loan_amount_affordable = initial_affordable_purchase_price - initial_total_downpayment_amount_affordable - initial_downpayment_assistance_amount
