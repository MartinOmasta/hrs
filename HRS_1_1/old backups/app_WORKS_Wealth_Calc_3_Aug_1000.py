...
with row1_col2:
    ...
    net_proceeds_fig.update_layout(
        xaxis_title='<b>Years</b>', 
        yaxis_title='<b>Net Proceeds / Wealth Built ($)</b>',
        margin=dict(l=40, r=20, t=20, b=40),
        yaxis=dict(autorange=True),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font=dict(size=14), itemclick="toggle", itemdoubleclick="toggleothers")
    )
...
with row2_col1:
    ...
    continuing_affordability_fig.add_trace(go.Scatter(x=proj['Year'], y=proj['ResaleContinuingAffordabilityPctOfAMIFixed'], mode='lines+markers', name='Required AMI % (Fixed)', line=dict(color='#F46A25', width=2), marker=dict(symbol='star', size=8), hovertemplate="Fixed - Year: %{x}<br>AMI Required: %{y:.1f}%<extra></extra>"))
    continuing_affordability_fig.add_trace(go.Scatter(x=proj['Year'], y=proj['ResaleContinuingAffordabilityPctOfAMIAMI'], mode='lines+markers', name='Required AMI % (AMI)', line=dict(color='#4363d8', width=2), marker=dict(symbol='cross', size=8), hovertemplate="AMI - Year: %{x}<br>AMI Required: %{y:.1f}%<extra></extra>"))
...
with row2_col2:
    ...
    clt_equity_fig.update_layout(
        xaxis_title='<b>Years</b>', 
        yaxis_title='<b>Retained Subsidy / CLT Equity ($)</b>',
        margin=dict(l=40, r=20, t=20, b=40),
        yaxis=dict(autorange=True),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font=dict(size=14))
    )
...
#saved_cols = ['Year', 'InitialMarketRateValue', 'InitialVALEFixedRateValue', 'InitialAMIRateValue', 'ResaleNetWealthBuiltFixed', 'ResaleContinuingAffordabilityPctOfAMIFixed', 'ResaleCLTEquityAmountFixed']
saved_cols = ['Year', 'InitialMarketRateValue', 'ResaleNetWealthBuiltFixed', 'InitialVALEFixedRateValue', 'ResaleCumMortgagePayment_P_And_I_Afforable', 'ResaleCumPropertyTaxFixed', 'ResaleCumInsurance', 'ResaleCumHOA', 'ResaleCumGroundLease','InitialBuyerCashContributionDownpayment', 'InitialClosingCostsAffordable', 'ResaleSellingCostsFixed', 'InitialBalanceAffordable']
