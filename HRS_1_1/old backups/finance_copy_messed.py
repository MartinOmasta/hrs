...
def compute_projections(
    ...
    length_of_mortgage_in_months = 360
    ...
    proceeds_market = []
    proceeds_fixed = []
    proceeds_ami = [] # ADDED: Core AMI proceeds array
    
    # Equity Gains (Wealth Built)
    equity_gain_market = []
    equity_gain_fixed = []
    equity_gain_ami = [] # ADDED: Core AMI wealth built array
    
    # Lifecycle Cashflow Changes (Proceeds - Holding Costs)
    net_cashflow_change_market = []
    net_cashflow_change_fixed = []
    net_cashflow_change_ami = [] # ADDED: Core AMI cashflow change array
    ...
    for t in years:
        ...
        current_market_val = initial_market_value * ((1 + market_home_price_inflation_rate / 100) ** t)
        current_fixed_val = affordable_purchase_price * ((1 + vale_resale_fixed_index_pct / 100) ** t)
        
        # AMI-Indexed Resale Value
        current_ami_scalar = (1 + area_median_income_inflation / 100) ** t
        ...
        if m_elapsed == 0:
            ...
            cum_i_paid = (avail_mortgage_payment_monthly * 360) - max_loan_amount
        else:
            # Standard Amortization Payoff Formula
            loan_bal = max_loan_amount * (((1 + mortgage_rate_monthly) ** 360) - ((1 + mortgage_rate_monthly) ** m_elapsed)) / (((1 + mortgage_rate_monthly) ** 360) - 1)
        ...
        selling_costs_market = current_market_val * (selling_cost_pct / 100)
        selling_costs_fixed = current_fixed_val * (selling_cost_pct / 100)
        selling_costs_ami = current_ami_val * (selling_cost_pct / 100) # ADDED
        ...
        net_proc_ami = max(0.0, current_ami_val - loan_bal - selling_costs_ami) # ADDED

        proceeds_market.append(net_proc_market)
        proceeds_fixed.append(net_proc_fixed)
        proceeds_ami.append(net_proc_ami) # ADDED
        ...
        equity_gain_ami.append(net_proc_ami - downpayment_amount) # ADDED
        ...
        net_cashflow_change_ami.append(net_proc_ami - downpayment_amount - total_payments) # ADDED
    ...
    # END # Section B: Resale Calculations ################################################################################
    ...
    financials_df = pd.DataFrame(
        {
            ...
            "VALEFixedPriceValue": np.round(vale_fixed_rate_value, 2),
            ...
            "NetCashflowChangeAMI": np.round(net_cashflow_change_ami, 2), # ADDED
